    ...
    useEffect(() => {
        async function fetchCurrentGame() {
            ...
            try {
                ...
                const runningGame = response.games.find((g) => g.status.toLowerCase() === "running")

                if (runningGame) {
                    // Check if user's team is actually in this game
                    // STUDENT role can only call get() for their own team, not list()
                    try {
                        await api.games.teams.get(runningGame.id, user.teamId)
                        // If we get here, team is in the game
                        setCurrentGame(runningGame)
                    } catch {
                        // Team not in game or error
                        setCurrentGame(null)
                    }
                } else {
                    setCurrentGame(null)
