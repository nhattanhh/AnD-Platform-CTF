...
import { useScoreboardWS } from "@/hooks/use-scoreboard-ws"
...
export default function LiveScoreboardPage() {
    ...
    // Use WebSocket hook for real-time scoreboard updates
    const { scoreboard: scoreboardData, isLoading, isConnected, refresh } = useScoreboardWS(selectedGameId)
    const scoreboard = scoreboardData?.entries || []
        ...
        async function fetchGames() {
            try {
                ...
                const activeGames = response.games.filter(
                    g => g.status.toLowerCase() === "running" || g.status.toLowerCase() === "paused" || g.status.toLowerCase() === "finished"
                )
                setGames(activeGames)
                const runningGame = activeGames.find(g => g.status.toLowerCase() === "running")
                if (runningGame) {
            ...
            <div className="mx-auto max-w-5xl px-4 py-8">
                ...
                <div className="flex items-center gap-4 mb-6">
                            ...
                            {games.length === 0 ? (
                                ...
                                games.map((game) => (
                                    <SelectItem key={game.id} value={game.id}>
                                        {game.name}
                                        {game.status.toLowerCase() === "running" && " (LIVE)"}
                                    </SelectItem>
                                ))
                            )}
                        </SelectContent>
                    ...
                    <div className="flex items-center gap-1 text-sm">
                        {isConnected ? (
                            <><Wifi className="h-4 w-4 text-emerald-500" /><span className="text-emerald-500">Live</span></>
                        ) : (
                            <><WifiOff className="h-4 w-4 text-muted-foreground" /><span className="text-muted-foreground">Connecting...</span></>
                        )}
                    </div>

                    {selectedGame && (
                        <div className="flex items-center gap-3 ml-auto">
                            <Badge variant={selectedGame.status.toLowerCase() === "running" ? "default" : "secondary"}>
