...
-- ==============================================================
-- 13. SCOREBOARD TRIGGER FOR REAL-TIME UPDATES
-- ==============================================================
-- Function to notify scoreboard changes via PostgreSQL LISTEN/NOTIFY
CREATE OR REPLACE FUNCTION notify_scoreboard_update()
RETURNS TRIGGER AS $$
DECLARE
    payload JSON;
BEGIN
    -- Build payload with game_id and team_id
    ...
    PERFORM pg_notify('scoreboard_updated', payload::text);
...
DROP TRIGGER IF EXISTS scoreboard_update_trigger ON scoreboard;
...
CREATE TRIGGER scoreboard_update_trigger
    ...
    FOR EACH ROW
    EXECUTE FUNCTION notify_scoreboard_update();
