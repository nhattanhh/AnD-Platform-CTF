...
async def submit_flag(
    ...
    await event_manager.broadcast_submission_event(
        game_id=game_team.game_id,
        team_id=game_team.team_id,
        status=SubmissionStatus.ACCEPTED.value,
        points=points,
        flag_owner_team_id=flag.team_id,
    )
    
    # Broadcast scoreboard update
    scoreboard_entries = await db.execute(
        select(Scoreboard).where(Scoreboard.game_id == game_team.game_id).order_by(Scoreboard.rank)
    )
    entries = [
        {
            "team_id": s.team_id,
        ...
        for s in scoreboard_entries.scalars().all()
    ]
    
    # Get game info for broadcast
    game = await db.execute(select(Game).where(Game.id == game_team.game_id))
    game = game.scalar_one_or_none()
    
    await event_manager.broadcast_scoreboard_update(
        game_id=game_team.game_id,
        entries=entries,
        game_name=game.name if game else None,
        current_tick=game.current_tick if game else None,
    )
