...
from src.models import Game, GameStatus, Tick, TickStatus, FlagType, GameTeam, Scoreboard
...
class TickWorker:
    ...
    async def execute_tick(self, db, game: Game, tick_number: int):
        ...
        await event_manager.broadcast_tick_event(
            game_id=game.id,
            tick_number=tick_number,
        ...
        scoreboard_entries = await db.execute(
            select(Scoreboard).where(Scoreboard.game_id == game.id).order_by(Scoreboard.rank)
        )
        entries = [
            {
                "team_id": s.team_id,
